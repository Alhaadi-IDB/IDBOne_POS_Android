...
import com.idb.idbonepos.PrintStatus
import com.idb.idbonepos.model.PrintReport
...
fun WebViewScreen(
    ...
    var isFullscreen by remember { mutableStateOf(true) }//make fullscreen as default
            ...
            WebView(ctx).apply {
                ...
                webChromeClient = object : WebChromeClient() {
                    ...
                    override fun onConsoleMessage(consoleMessage: android.webkit.ConsoleMessage): Boolean {
                        ...
                        // Filter out mixed content warnings and other non-critical messages
                        if (shouldIgnoreError(message)) {
                            return super.onConsoleMessage(consoleMessage)
                        }
                        // Only show ERROR level messages or likely JavaScript errors
...
private fun shouldIgnoreError(message: String): Boolean {
    val lower = message.lowercase()
    return lower.contains("invalid left hand side assignment") ||
        lower.contains("invalid left-hand side assignment") ||
        lower.contains("invalid left hand side in assignment") ||
        lower.contains("invalid left-hand side in assignment") ||
        lower.contains("left-hand side in assignment") ||
        lower.contains("left hand side in assignment") ||
        lower.contains("invalid left hand side") ||
        lower.contains("invalid left-hand side") ||
        lower.contains("invalid assignment") ||
        lower.contains("assignment to constant") ||
        lower.contains("cannot assign to") ||
        lower.contains("mixed content") ||
...
private class PrintBridgeJsInterface(
    ...
    @JavascriptInterface
    fun sendPrintToAndroid(ipAddress: String?, pdfUrl: String?, type: String?) {//trigger from js web
        try {
            ...
            mainHandler.post {
                try {
                    ...
                    onError("Failed to process idbonepos request: ${e.message}")
                }
            }
        } catch (e: Exception) {
            Log.e("PrintBridgeJsInterface", "Error in sendPrintToAndroid: ${e.message}", e)
            mainHandler.post {
                onError("Failed to handle idbonepos request: ${e.message}")
            }
        }
    }
...
//    }
    ...
    fun reportError(message: String?) {
        try {
            ...
            mainHandler.post {
                try {
                    if (value.isNotBlank() && !shouldIgnoreError(value)) {
        ...
        try {
            ...
            mainHandler.post {
                try {
                    ...
                    onError("Failed to process idbonepos: ${e.message}")
                }
            }
        } catch (e: Exception) {
            Log.e("PrintBridgeJsInterface", "Error in handlePrint: ${e.message}", e)
